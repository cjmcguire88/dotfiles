import { deleteDB } from '../node_modules/idb/build/index.js';
...
async function observe(property, fn) {
  let wrapper;
  if (property) {
    let value;
...
function isGlobalPaused(options) {
  return options.paused.some(({ id }) => id === GLOBAL_PAUSE_ID);
}

export { ENGINES, GLOBAL_PAUSE_ID, SYNC_OPTIONS, Options as default, isGlobalPaused, observe, sync };
